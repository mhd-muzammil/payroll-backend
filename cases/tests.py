...
class CaseSyncTests(TestCase):
    ...
    # -- closures: whose is final, whose can be taken back --------------------

    def _sync_closed(self, ticket):
        """One sync tick reporting the ticket as closed upstream."""
        res = self.bot_client.post(
            "/api/cases/bulk_dispatch/",
            {
                "cases": [
    ...
    def test_a_closure_reported_upstream_is_reopened_when_the_call_is_rescheduled(self):
        """A carried call closed one evening and re-booked next morning must come
        back to the engineer. Making an upstream closure permanent would erase it
        from their list for good."""
        self._sync(["TKT-1"])
        self._sync_closed("TKT-1")

        case = Case.objects.get(external_ref="TKT-1")
        self.assertEqual(case.status, "completed")
        self.assertEqual(case.completion_source, Case.COMPLETED_BY_UPSTREAM)
        self.assertEqual(self._engineer_cases(), [])

        # Next morning it is back in the plan.
        self._sync(["TKT-1"])

        case.refresh_from_db()
        self.assertEqual(case.status, "assigned")
        self.assertIsNone(case.completed_at, "the old closure time must not linger")
        self.assertEqual(case.completion_source, "")
        self.assertEqual(len(self._engineer_cases()), 1, "engineer sees it again")

    def test_a_closure_the_engineer_made_is_never_reopened_by_a_sync(self):
        """The engineer reported the work done. No amount of upstream lag may
        put it back on their list."""
        self._sync(["TKT-1"])
        case = Case.objects.get(external_ref="TKT-1")
        for step in ("accept", "start_travel", "reached", "start_work", "complete"):
            res = self.engineer_client.post(f"/api/cases/{case.id}/{step}/")
        ...
        case.refresh_from_db()
        self.assertEqual(case.completion_source, Case.COMPLETED_BY_ENGINEER)

        # Upstream still lists it as scheduled, for several ticks.
        self._sync(["TKT-1"])
        self._sync(["TKT-1"])
    ...
    def test_an_upstream_closure_does_not_overwrite_the_engineers_attribution(self):
        self._sync(["TKT-1"])
        case = Case.objects.get(external_ref="TKT-1")
        for step in ("accept", "start_travel", "reached", "start_work", "complete"):
    ...
    def test_reassignment_clears_the_closure_for_the_new_engineer(self):
        other_user = User.objects.create_user(username="samim2", password="x", role="employee")
        Employee.objects.create(
            user=other_user, employee_name="Samim", branch="Chennai", salary=0
        )
        self._sync(["TKT-1"])
    ...
    def test_the_response_names_the_tickets_that_reached_nobody(self):
        res = self.bot_client.post(
            "/api/cases/bulk_dispatch/",
            {
                "cases": [
                    {"external_ref": "TKT-1", "title": "a", "engineer_name": "Praveen", "status": "assigned"},
                    {"external_ref": "TKT-LOST", "title": "b", "engineer_name": "Lava", "status": "assigned"},
                ]
            },
            format="json",
        )
        self.assertEqual(res.data["skipped"], 1)
        self.assertEqual(res.data["skipped_refs"], ["TKT-LOST"])

    def test_the_response_names_the_tickets_the_mirror_retracted(self):
        self._sync(["TKT-1", "TKT-2"])
        data = self._sync(["TKT-2"])

        self.assertEqual(data["cancelled"], 1)
        self.assertEqual(data["cancelled_refs"], ["TKT-1"])
